...
use Afroware\JwTauth\Exceptions\PayloadException;
...
class Payload implements ArrayAccess, Arrayable, Countable, Jsonable, JsonSerializable
{
     ...
     *
     * @var \Afroware\JwTauth\Claims\Collection
     */
    private $claims;
     ...
     * @param  \Afroware\JwTauth\Validators\PayloadValidator  $validator
     ...
     * @param  string  $claim
     *
     * @return \Afroware\JwTauth\Claims\Claim
     */
     ...
     * @param  mixed  $value
     *
     * @throws \Afroware\JwTauth\Exceptions\PayloadException
     */
    ...
    public function __call($method, $parameters)
    {
        if (preg_match('/get(.+)\b/i', $method, $matches)) {
            foreach ($this->claims as $claim) {
                if (get_class($claim) === 'Afroware\\JwTauth\\Claims\\'.$matches[1]) {
